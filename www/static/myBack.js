...
//---Online
//var apipath="http://w02.yeapps.com/postit/syncmobile_20190701/";
...
$(document).ready(function(){
	...
	if (localStorage.rep_type=='CM'){
		...
		if (localStorage.attendanceCheck=='' || localStorage.attendanceCheck==undefined){
		  ...
		  success: function(result) { 
			var syncResultArray = result.split('rdrd');
				...
				if (localStorage.synced=='YES'){
					...
					localStorage.attendanceCheck=syncResultArray[9];
...
function receiveAndIssu(){
	...
	//alert(apipath+'get_posm_sup?select_town='+localStorage.select_town+"&repID="+localStorage.p_repID)
	$.ajax({
			  url:apipath+'get_posm_sup?select_town='+localStorage.select_town+"&repID="+localStorage.p_repID,
			  success: function(result) {
	...
	var csmListTools=localStorage.toolsStr.split('fdfd');
	...
	cmListStr+='</select>'
	localStorage.ToolsListCM=cmListStr;
	$("#cmToolsRecRec").empty();
	$("#cmToolsRecRec").append(localStorage.ToolsListCM).trigger('create');
...
function submitDataTargetCM(){
	$(".sucMsgTarget").text('');
	$("#btn_submit_receive").hide();
	//alert(localStorage.target_li);
	if (localStorage.target_li=='' || localStorage.target_li==undefined){
		
		$(".errorChk").text("Required Values");
		$("#btn_submit_receive").show();
		
	}else{
	
	$("#TargetSubmitbufferImage").show();			
		//alert(apipath+"submitDataCmTarget?&syncCode="+localStorage.p_sync_code+"&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&mobileNo="+localStorage.mobileNo+"&select_town="+localStorage.select_town+"&recordDataTargget="+localStorage.target_li);	
		$.ajax({
			type: 'POST',
			url:apipath+"submitDataCmTarget?&syncCode="+localStorage.p_sync_code+"&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&mobileNo="+localStorage.mobileNo+"&select_town="+localStorage.select_town+"&recordDataTargget="+localStorage.target_li,
			...
			success: function(result) {	
				...
				if(resultArray[0]=='Success'){
					...
					localStorage.target_li='';
					$("#target_table").empty();
					$("#targetPOSMSelect").empty();
					$("#targetPOSMSelect").append(localStorage.posmCodeTarget).trigger('create');	
					$(".errorChk").text("");
					$(".sucMsgTarget").show();
					$("#allHideTarget").hide();
...
function salfie_next_page(){
		...
		if(salfie_image_name==''){
			...
			if (salfie_image_path.length < 10){
				...
				//alert(apipath+'syncAttendanceData?select_town='+localStorage.town+"&repID="+localStorage.p_repID+'&salfie_data='+salfie_image_name+'&selfie_lat='+selfie_lat+'&selfie_long='+selfie_long+'&attendance='+attendance,);
				
				$.ajax({
				//type: 'POST',
				url: apipath+'syncAttendanceData?select_town='+localStorage.town+"&repID="+localStorage.p_repID+'&salfie_data='+salfie_image_name+'&selfie_lat='+selfie_lat+'&selfie_long='+selfie_long+'&attendance='+attendance,
				 success: function(result) {	
						...
						if (result==''){
							...
							localStorage.attendanceCheck="Day Start"
			  ...
			  success: function(result) {
				var resultArray = result.split('rdrd');
					if (resultArray[0]=='Success'){	
						...
						localStorage.csmListTools=resultArray[6];
						localStorage.toolsStr=resultArray[7];
						
						var csmListTools=localStorage.csmListTools.split('fdfd');
						...
						cmListStr+='</select>'
						localStorage.cmListToolsList=cmListStr;
						$("#csmListToolsRecRec").empty();
						$("#csmListToolsRecRec").append(localStorage.cmListToolsList).trigger('create');
...
function submitData_ReceiveList(){
	...
	if (receiveQtyR!=0){
		if ( receiveQty <= 0){
			...
			if((localStorage.receive_li==undefined)||(localStorage.receive_li=='')){	
				
				localStorage.receive_li=posm_code+'||'+posm_type+'||'+brand+'||'+dueRec+'||'+alcId+'||'+receiveQtyR+'||'+damageQtyR;
				$("#posm_code").val('');
				$("#posm_type").val('');
				$("#brand").val('');	
				$("#dueRec").val('');
				...
				if( localStorage.receive_li.indexOf(posm_code) >-1 ){
					$(".errorChk").html('Already Exits');	
				}else{
					localStorage.receive_li+='<rdrd>'+posm_code+'||'+posm_type+'||'+brand+'||'+dueRec+'||'+alcId+'||'+receiveQtyR+'||'+damageQtyR;
					//alert(localStorage.receive_li);
					$("#posm_code").val('');
					$("#posm_type").val('');
					$("#brand").val('');	
					$("#dueRec").val('');
...
function receive_list_show(){
	...
	if ((localStorage.receive_li==undefined) || (localStorage.receive_li=="")){
		...
		var medicien_rx=localStorage.receive_li.split('<rdrd>');
...
function rec_defe_del(id){
		...
		var iStrS=localStorage.receive_li
...
function submitData_TargetList(){
	...
	$(".errorChk").html('');
	var cm_id=$("#csmTargetRecRec").val();
	var posm_code=$("#targetPOSMS").val();
	var targetQty=$("#targetQty").val();
	var targetDate=$("#targetDate").val();
	
	var posm_type=$("#posm_typeT").val();
	//alert(posm_code+'=='+receiveQtyR +'=='+damageQtyR);
	var brand=$("#brandT").val();
	var alcQty=$("#allocationT").val();
	var dueTarg=$("#dueTarget").val();
	var alcId=$("#alc_idT").val();
	...
	if(cm_id==''){
		$(".errorChk").html('Required CM ID');	
	}else if(posm_code=='' || posm_code==undefined){
		$(".errorChk").html('Please Select POSM Again');	
	}else if (targetQty==0){
		...
		if ( targetQty <= 0){
			...
			if((localStorage.target_li==undefined)||(localStorage.target_li=='')){	
				
				localStorage.target_li=posm_code+'||'+cm_id+'||'+posm_type+'||'+brand+'||'+alcQty+'||'+dueTarg+'||'+alcId+'||'+targetQty+'||'+targetDate;
				...
				if( localStorage.target_li.indexOf(posm_code) >-1 ){
					$(".errorChk").html('Already Exits');	
				}else{
					localStorage.target_li+='<rdrd>'+posm_code+'||'+cm_id+'||'+posm_type+'||'+brand+'||'+alcQty+'||'+dueTarg+'||'+alcId+'||'+targetQty+'||'+targetDate;
					//alert(localStorage.receive_li);
					$("#targetPOSMS").val('');
					$("#targetQty").val('');
					$("#targetDate").val('');	
...
function target_list_show(){
	...
	if ((localStorage.target_li==undefined) || (localStorage.target_li=="")){
		...
		var medicien_rx=localStorage.target_li.split('<rdrd>');
		
		var medicienStr='';
		medicienStr += '<tr style="font-size:12px;"><th>CM_ID POSM</th><th>Tar_Qty</th><th>Tar_Date</th></tr>'
		for(i=0;i<medicien_rx.length;i++){
			medicienList=medicien_rx[i].split('||');
			
			posmCodeT=medicienList[0];
			cm_id=medicienList[1];
			targetQty=medicienList[7];
			targetDate=medicienList[8];
			
			
			medicienStr+='<tr id="'+posmCodeT+'" style="font-size:12px;"><td width="60%">'+cm_id+'</br>'+posmCodeT+'</td><td width="9%" style="text-align:center;">'+targetQty+'</td><td width="20%" style="text-align:center;">'+targetDate+'</td><td width="5%"><input type="button" onClick="rec_target_del(\''+posmCodeT+'\')" value="X" style="background-color: #0E5887;color: #fff;"></td></tr>'
			
		}
		
		$("#target_table").empty();
		$("#target_table").append(medicienStr);
...
function rec_target_del(id){
	...
	var successData = id.split('|');
		var iStrS=localStorage.target_li
		for(j=0;j<successData.length;j++){
			var listS1='';
			iStrS=iStrS.split('<rdrd>');
			iLenS=iStrS.length
...
function submit_data_receive(){
	...
	if(localStorage.receive_li=='' || localStorage.receive_li==undefined){
		
		$(".errorChk").text("Required Value");
		$("#btn_submit_receive").show();
	...
	$("#receiveSubmitbufferImage").show();			
		//alert(apipath+"submitData_receive?&syncCode="+localStorage.p_sync_code+"&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&mobileNo="+localStorage.mobileNo+"&select_town="+localStorage.select_town+"&imageName="+imageName+"&recordData_list="+localStorage.receive_li);	
		$.ajax({
			type: 'POST',
			url:apipath+"submitData_receive?&syncCode="+localStorage.p_sync_code+"&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&mobileNo="+localStorage.mobileNo+"&select_town="+localStorage.select_town+"&imageName="+imageName+"&recordData_list="+localStorage.receive_li,
			...
			success: function(result) {	
				...
				/*}else if (result=='Faild'){
					...
					/*var failData = resultArray[1].split('|');
					for(i=0;i<failData.length;i++){
						rec_defe_del(failData[i]);
					}*/
						rec_defe_del(resultArray[1]);
...
function cmToolsSubmit(){
	...
	$(".sucMsgTools").text('');
	var csmListToolsRecRec=$("#csmToolsRecRec").val();
	var cmToolsRecRec=$("#cmToolsRecRecData").val();
	var toolqty=$("#toolqty").val();
	
	if(csmListToolsRecRec==''){
		$(".errorChk").text("Required CM");
		...
		//alert(apipath+"submitData_toolsRec?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&select_town="+localStorage.select_town+"&cmCode="+csmListToolsRecRec+"&toolsName="+cmToolsRecRec+"&toolqty="+toolqty);	
		$.ajax({
			type: 'POST',
			url:apipath+"submitData_toolsRec?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&select_town="+localStorage.select_town+"&cmCode="+csmListToolsRecRec+"&toolsName="+cmToolsRecRec+"&toolqty="+toolqty,
...
function targetSetCMGo(){
	...
	for (i=0;i<posmCodeCmSup.length;i++){	
	 ...
	 posmStrRec +='</select>'
	posmStrRec =posmStrRec
	localStorage.posmCodeTarget=posmStrRec;
	$("#targetPOSMSelect").empty();
	$("#targetPOSMSelect").append(localStorage.posmCodeTarget).trigger('create');
	...
	cmListStr+='</select>'
	localStorage.cmListTargetsList=cmListStr;
	$("#csmListTargetRecRec").empty();
	$("#csmListTargetRecRec").append(localStorage.cmListTargetsList).trigger('create');
...
function outlet(outletIDName){
		...
		if (localStorage.synced!='YES'){
			...
			if(posmAuditOutSearch==''){
				...
				//alert(apipath+"getOutletPosmAuditor?&posmAuditOutSearch="+posmAuditOutSearch+"&repID="+localStorage.p_repID);
				$.ajax({
				type: 'POST',
				url:apipath+"getOutletPosmAuditor?&posmAuditOutSearch="+posmAuditOutSearch+"&repID="+localStorage.p_repID+"&town="+localStorage.select_town,
			...
			if(localStorage.rep_type =='CM'){
				...
				$("#townJoinCall").html('Town	:		' + localStorage.select_town.replace('|','-'));
...
function summaryCM(){
	...
	$("#psInstallSummary").hide();
	$(".errorChk").text("");
	$("#cmIDNameSummmary").html('Rep	:		' + localStorage.p_repID+','+localStorage.repName);				
	url="#pageSummary";				
	$.mobile.navigate(url);	
}
...
function psInsSumB(){
	...
	//alert(apipath+"posmInstallationSummary?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&select_town="+localStorage.select_town);
	$.ajax({
			type: 'POST',
			url:apipath+"posmInstallationSummary?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&select_town="+localStorage.select_town,
			...
			success: function(result) {
				...
				$("#bufferImageTARvsACH").hide();				
				localStorage.posmInstallData=getResult[1];	
				
				var stockRPT=localStorage.posmInstallData.split('rdrd');			
...
function leave(){
	...
	$("#cmIDName").html('Rep	:		' + localStorage.p_repID+','+localStorage.repName);	
	$(".errorChk").text("");
	$(".sucChk").text("");			
	url="#leaveHome";				
	$.mobile.navigate(url);	
...
function anualLeaveSub(){
	...
	//alert(apipath+'anualLeaveSubmit?select_town='+localStorage.select_town+"&repID="+localStorage.p_repID+"&fr_dateLeave="+fr_dateLeave+"&to_dateLeave="+to_dateLeave+"&anual_leave="+anual_leave);
$.ajax({
	  url:apipath+'anualLeaveSubmit?select_town='+localStorage.select_town+"&repID="+localStorage.p_repID+"&fr_dateLeave="+fr_dateLeave+"&to_dateLeave="+to_dateLeave+"&anual_leave="+anual_leave,
	 success: function(result) {			
				if(result=='Success'){
					$('#bufferImageLeaveR').hide();
					$("#anualLeaveSub").hide();
...
function sickLeaveSub(){
	...
	//alert(apipath+'sickLeaveSubmit?select_town='+localStorage.select_town+"&repID="+localStorage.p_repID+"&fr_dateLeaveS="+fr_dateLeaveS+"&to_dateLeaveS="+to_dateLeaveS+"&sick_leave="+sick_leave);
$.ajax({
	  url:apipath+'sickLeaveSubmit?select_town='+localStorage.select_town+"&repID="+localStorage.p_repID+"&fr_dateLeaveS="+fr_dateLeaveS+"&to_dateLeaveS="+to_dateLeaveS+"&sick_leave="+sick_leave,
	 success: function(result) {			
				if(result=='Success'){
					$('#bufferImageLeaveS').hide();
					$("#sickLeaveSub").hide();
...
function recReport(){
	...
	//alert (apipath+"receive_report?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&townName="+town)
	$("#bufferImageRecR").show();
	$.ajax({
			type: 'POST',
			url:apipath+"receive_report?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&townName="+town,																																																				
			success: function(result) {
				getResult=result.split('||');	
				if(getResult[0]=='Success'){
...
function recUsage(){
	...
	//alert (apipath+"usage_report?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&townName="+localStorage.select_town+"&fr_date="+fr_date+"&to_date="+to_date)
	$("#bufferImageUsaR").show();
	$.ajax({
			type: 'POST',
			url:apipath+"usage_report?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&townName="+localStorage.select_town+"&fr_date="+fr_date+"&to_date="+to_date,
...
function stockReport(){
	...
	//alert (apipath+"stock_report?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&townName="+localStorage.select_town+"&selectPOSM="+selectPOSM)
	$("#bufferImageStockR").show();
	$.ajax({
			type: 'POST',
			url:apipath+"stock_report?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&townName="+localStorage.select_town+"&selectPOSM="+selectPOSM,
...
function summary_report(){
	...
	//alert (apipath+"agency_report?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&townName="+town)
	$('#menuBufferingImage').show();
	$("#bufferImageAgencyA").show();
	$.ajax({
			type: 'POST',
			url:apipath+"agency_report?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&townName="+town,
...
function stockAgency(){
	...
	//alert (apipath+"stock_RAgency?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&townName="+town)
	$('#menuBufferingImage').show();
	$("#agencySTRimage").show();
	$.ajax({
			type: 'POST',
			url:apipath+"stock_RAgency?&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&townName="+town,
...
function submit_data_defective(){
	...
	if (!regg.test(a_qty)){
		...
		//alert(apipath+"submitData_defective?&syncCode="+localStorage.p_sync_code+"&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&mobileNo="+localStorage.mobileNo+"&town="+town+"&posmCode="+posmCode+"&uposm_type="+uposm_type+"&ubrand="+ubrand+"&baUsage="+baUsage+"&a_qty="+a_qty+"&alcId="+alcId+"&imageName6="+imageName6);
		$.ajax({
			type: 'POST',
			url:apipath+"submitData_defective?&syncCode="+localStorage.p_sync_code+"&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&mobileNo="+localStorage.mobileNo+"&town="+town+"&posmCode="+posmCode+"&uposm_type="+uposm_type+"&ubrand="+ubrand+"&baUsage="+baUsage+"&a_qty="+a_qty+"&alcId="+alcId+"&imageName6="+imageName6,
...
function leaveManageSUP(){
	
	
	$('#menuBufferingImage').show();
	$("#agencySTRimage").show();
	//alert(apipath+"leaveManageSUPData?&repID="+localStorage.p_repID+"&select_town="+localStorage.select_town);
	$.ajax({
			type: 'POST',
			url:apipath+"leaveManageSUPData?&repID="+localStorage.p_repID+"&select_town="+localStorage.select_town,
			...
			success: function(result) {
				...
				$('#menuBufferingImage').hide();
				$("#agencySTRimage").hide();
				localStorage.adadsasa=getResult[1];	
					if(localStorage.adadsasa!=''){
					var stockRPA=localStorage.adadsasa.split('rdrd');			
					var cmRouteSTA='<table id="leaveTable">';
						cmRouteSTA += '<tr style="font-size:12px;"><th>CM ID</th><th>Date</th><th>Day</th><th></th><th></th></tr>'
						for (i=0;i<stockRPA.length;i++){	
							medicienList=stockRPA[i].split('|');
							rowID=medicienList[0];
							cmID=medicienList[1];
							frDate=medicienList[2];
							toDate=medicienList[3];
							days=medicienList[4];	
												
							cmRouteSTA += '<tr id="'+rowID+'" style="font-size:12px;"><td width="20%">'+cmID+'</td><td width="40%">'+frDate+' To '+toDate+'</td><td width="10%" style="text-align:center;">'+days+'</td><td width="10%"><input type="button" onClick="rec_defe_deldddd(\''+rowID+'\')" value="X" style="background-color:#f00; color:#fff;"></td><td width="15%"><input type="button" onClick="rec_defe_deldddd(\''+rowID+'\')" value="Approve" style="background-color: #0E5887;color: #fff;"></td></tr>'
						...
						$("#leaveViewTable").empty();
						$(".errorChk").text("No Leave Request In Town");
						}
...
function uploadShopeForntPhoto(imageURI, ShopeForntImageA) { 	
	var options = new FileUploadOptions();
    options.fileKey="upload";
...
function joinCallSubmitDataAll(){
	...
	alert(apipath+"submitData_joinCallAllData?&select_town="+localStorage.select_town+"&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&mobileNo="+localStorage.mobileNo+"&joinNo1="+joinNo1+"&joinNo2="+joinNo2+"&joinNo3="+joinNo3+"&joinNo4="+joinNo4+"&shopeFornt_name="+shopeFornt_name+"&getFixedDisplayBefor_name="+getFixedDisplayBefor_name+"&getFixedDisplayAfter_name="+getFixedDisplayAfter_name+"&routeNameFixed="+rName+"&outletNameFixed="+outletCodeFxiedDisplayDala);
		$.ajax({
			type: 'POST',
			url:apipath+"submitData_joinCallAllData?&select_town="+localStorage.select_town+"&repID="+localStorage.p_repID+"&repName="+localStorage.repName+"&mobileNo="+localStorage.mobileNo+"&joinNo1="+joinNo1+"&joinNo2="+joinNo2+"&joinNo3="+joinNo3+"&joinNo4="+joinNo4+"&shopeFornt_name="+shopeFornt_name+"&getFixedDisplayBefor_name="+getFixedDisplayBefor_name+"&getFixedDisplayAfter_name="+getFixedDisplayAfter_name+"&routeNameFixed="+rName+"&outletNameFixed="+outletCodeFxiedDisplayDala,
			...
			success: function(result) {			
				if(result=='Success'){
					...
					$(".sucMsgJoinCallSub").text('Successfully Submitted');
					//document.getElementById('myImageD').src = '';
