...
function supPosmCreateCombo(){
	
	var select_town=$("#supreceivePOSMSelectA").val();
	
	$('#menuBufferingImage').hide();
	alert(apipath+'get_posm_sup?select_town='+select_town+"&repID="+localStorage.p_repID)
	$.ajax({
			  url:apipath+'get_posm_sup?select_town='+select_town+"&repID="+localStorage.p_repID,
			  success: function(result) {
				var resultArray = result.split('rdrd');
					if (resultArray[0]=='Success'){
						$('#menuBufferingImage').show();	
						localStorage.p_posmCodeCmSupNew=resultArray[1];
						alert(localStorage.p_posmCodeCmSupNew);
						if(localStorage.p_posmCodeCmSupNew!=''){			
					//====CM/Sup	
					var posmCodeCmSup=localStorage.p_posmCodeCmSupNew.split('fdfd');
					var posmStrRec = '<select style="background:none;" id="posmCodeRecNewSup" class="auto_break" name="posmCodeRecNewSup" onchange="alloDetails();"><option selected="selected" value="">Select POSM</option>'
...
function submitDataTargetCM(){
	...
	if (localStorage.p_target_li=='' || localStorage.p_target_li==undefined){
	...
	$("#TargetSubmitbufferImage").show();			
		//alert(apipath+"submitDataCmTarget?&syncCode="+localStorage.p_sync_code+"&repID="+localStorage.p_repID+"&repName="+localStorage.p_repName+"&mobileNo="+localStorage.p_mobileNo+"&select_town="+localStorage.p_select_town+"&recordDataTargget="+localStorage.p_target_li);	
		$.ajax({
			type: 'POST',
			url:apipath+"submitDataCmTarget?&syncCode="+localStorage.p_sync_code+"&repID="+localStorage.p_repID+"&repName="+localStorage.p_repName+"&mobileNo="+localStorage.p_mobileNo+"&select_town="+localStorage.p_select_town+"&recordDataTargget="+localStorage.p_target_li,
			  ...
			  success: function(result) {
				var resultArray = result.split('rdrd');
					if (resultArray[0]=='Success'){	
						...
						localStorage.p_toolsStr=resultArray[7];
...
function receive(){
		...
		$('#menuBufferingImage').hide();
	alert(apipath+'get_Town_sup?repID='+localStorage.p_repID)
	$.ajax({
			  url:apipath+'get_Town_sup?repID='+localStorage.p_repID,
			  success: function(result) {
				var resultArray = result.split('rdrd');
					if (resultArray[0]=='Success'){
						$('#menuBufferingImage').show();	
						localStorage.p_townListRecSup=resultArray[1];
						if(localStorage.p_townListRecSup!=''){			
					//====CM/Sup	
					var csmListTools=localStorage.p_townListRecSup.split('fdfd');
					var cmListStr = '<select id="supreceivePOSMSelectA" class="auto_break" name="supreceivePOSMSelectA"  onChange="supPosmCreateCombo();" style="display:inline-block;"><option selected="selected" value="">Select Town</option>'
					for (j=0;j<csmListTools.length;j++){	
						cmListStr += '<option value="'+csmListTools[j]+'">'+csmListTools[j]+'</option>'
					}
					cmListStr+='</select>'
...
function alloDetails(){
	...
	if(localStorage.p_rep_type=="CM"){
		...
		var posmCode=$("#posmCodeRecNewSup").val();
		var town = $("#supreceivePOSMSelectA").val();
		}
	alert(posmCode+"-"+town);	
	if(posmCode==""|| posmCode==null){
	...
	if(cm_id==''){
		...
		if ( targetQty <= 0){
			...
			if((localStorage.p_target_li==undefined)||(localStorage.p_target_li=='')){	
				...
				localStorage.p_target_li=posm_code+'||'+cm_id+'||'+posm_type+'||'+brand+'||'+alcQty+'||'+dueTarg+'||'+alcId+'||'+targetQty+'||'+targetDate;
				...
				if( localStorage.p_target_li.indexOf(posm_code) >-1 ){
					...
					localStorage.p_target_li+='<rdrd>'+posm_code+'||'+cm_id+'||'+posm_type+'||'+brand+'||'+alcQty+'||'+dueTarg+'||'+alcId+'||'+targetQty+'||'+targetDate;
					//alert(localStorage.p_receive_li);
					$("#targetPOSMS").val('');
					$("#targetQty").val('');
					$("#targetDate").val('');	
...
function submit_data_receive(){
	...
	/*}else if((a_qty != '' || a_qty==0) && imageName6 == ''){
		...
		alert(apipath+"submitData_receive?&syncCode="+localStorage.p_sync_code+"&repID="+localStorage.p_repID+"&repName="+localStorage.p_repName+"&mobileNo="+localStorage.p_mobileNo+"&select_town="+town+"&posmCode="+posm_code+"&posm_type="+posm_type+"&brand="+brand+"&dueRec="+dueRec+"&received="+received+"&imageName="+imageName+"&alcId="+alcId+"&def_qty="+def_qty+"&imageName6="+imageName6);	
		$.ajax({
			type: 'POST',
...
function cmToolsSubmit(){
	...
	if(csmListToolsRecRec==''){
		...
		//alert(apipath+"submitData_toolsRec?&repID="+localStorage.p_repID+"&repName="+localStorage.p_repName+"&select_town="+localStorage.p_select_town+"&cmCode="+csmListToolsRecRec+"&toolsName="+cmToolsRecRec+"&toolqty="+toolqty);	
		$.ajax({
			type: 'POST',
			url:apipath+"submitData_toolsRec?&repID="+localStorage.p_repID+"&repName="+localStorage.p_repName+"&select_town="+localStorage.p_select_town+"&cmCode="+csmListToolsRecRec+"&toolsName="+cmToolsRecRec+"&toolqty="+toolqty,
...
function usages(outletSep){
	...
	$('#menuBufferingImage').show();	
		...
		//alert(apipath+"getRoutetest?&townCodeName="+localStorage.p_select_town);
		$.ajax({
			type: 'POST',
			url:apipath+"getRoutetest?&townCodeName="+localStorage.p_select_town,
				...
				if(resultArray[0]=='Success'){
					...
					for (i=0;i<routeListSupALL.length;i++){	
						...
						if (routeDate==current_date){
							routeStr +='<label style="background:#81C0C0"><input type="radio" name="RadioRoute" value="'+routeName+'" id="RadioGroup1_0"> '+routeName+'</label>'
						}else if((routeDate==bak_date1)||(routeDate==bak_date2)||(routeDate==bak_date3)){
							routeStr +='<label><input type="radio" name="RadioRoute" value="'+routeName+'" id="RadioGroup1_0"> '+routeName+'</label>'
						}else{
							routeStr +='<label style="background:#ddeeee"><input type="radio" name="RadioRoute" value="'+routeName+'" id="RadioGroup1_0"> '+routeName+'</label>'
						}						
					}
					//routeStr +=routeStr;
					...
					//$('#routeList').html(localStorage.p_routeListSup);
...
function syncRoute() { 
	...
	localStorage.routeIDName=selected_route;
	if (routeDate==today_get){
		localStorage.selectedRoute=selected_route;
	...
	if(localStorage.selectedRoute!=undefined){
		//$("#errorChk").html(apipath+'sync_route?mobileNo='+localStorage.p_mobileNo+'&cm_pass='+localStorage.p_SyncPass+'&synccode='+localStorage.p_sync_code+'&route='+localStorage.selectedRoute);
	//======================================	
									...
									$("#RSButton").show();
										...
										for (var o=0; o < outletSingleTtotal-1; o++){
											...
											outletStringShow=outletStringShow+'<label ><table width="100%" border="0"> <tr> <td width="5%">'+
															'<input type="radio" name="RadioOutlet" value="'+outletID+'rdrd'+schedule_date+'"></td><td width="60%">'+outletName +' | '+ outletID +' | '+  schedule_date +' | '+channel +'</td> </tr></table></label>'
										...
										$("#bufferImageRoute").hide();
										$("#RSButton").show();
										//alert(schedule_date+'=='+sync_date_get);
										if (schedule_date==sync_date_get){
...
function select_outlet() { 		
		...
		//alert(selected_outletID+'===='+selected_date_get);
...
//==========Create Promotion Display list	
			...
			for (var i=0; i < qpdsSingleTotal-1; i++){
				...
				var qpds_image_div='qpds_image_div_'+i_text
				var qpds_image_div_hidden='qpds_image_div_hidden_'+i_text
				var qpds_image_name_hidden='qpds_image_name_hidden_'+i_text
				...
				qpdsStringShow=qpdsStringShow+
					...
					'<input type="hidden" name="'+ qpds_image_name_hidden +'" id="'+ qpds_image_name_hidden +'" value="" >'+
...
function fdisplay_ready_data() { 
	...
	for (var i=0; i < localStorage.fdisplaySlabTotal-1; i++){
		...
		//alert(fdSLfdisplay_image_path+'=='+fdSLfdisplay_image_name);
...
function fdisplay_page_set() { 
	if (localStorage.fdisplay_data_ready.length > 10){
		...
		for (var i=0; i < localStorage.fdisplaySlabTotal-1; i++){
			...
			//alert(slabfdisplay+'=='+fdisplayTotal+'=='+fdisplayImg+'=='+fdisplayImg_path+'=='+fdisplayImg_before+'=='+fdisplayImg_path_before);
...
function sRoute(routeName){
	...
	rName=routeName;
			...
			//alert(apipath+"getOutlet?&routeName="+rName+"&townCode="+localStorage.p_select_town+"&outletSeperator="+outletSeperator);
			//$('#bufferImageOutlet').show();	
			$('#bufferImageRoute').show();
			$.ajax({
			type: 'POST',
			url:apipath+"getOutlet?&routeName="+rName+"&townCode="+localStorage.p_select_town+"&outletSeperator="+outletSeperator,
	...
	$.ajax({
			...
			success: function(result) {	
				...
				if (recStatus=='Success'){	
					...
					$("#anualLeaveDiv").hide();	
					$("#bufferImageanualLeave").hide();
					$(".errorChk").text("Leave Record Empty");
				}
			}
		})	
...
function recUsage(){
	...
	$.ajax({
			type: 'POST',
			url:apipath+"usage_report?&repID="+localStorage.p_repID+"&repName="+localStorage.p_repName+"&townName="+localStorage.p_select_town+"&fr_date="+fr_date+"&to_date="+to_date,
			...
			success: function(result) {
				...
				var usatable='<table id="usageRepp">';
					...
					var townName=town;
					var repid=localStorage.p_repID;
					$("#townNameU").html ("Town	:		"+localStorage.p_select_town);
...
function stockReport(){
	...
	//alert (apipath+"stock_report?&repID="+localStorage.p_repID+"&repName="+localStorage.p_repName+"&townName="+localStorage.p_select_town+"&selectPOSM="+selectPOSM)
	$("#bufferImageStockR").show();
	$.ajax({
			type: 'POST',
			url:apipath+"stock_report?&repID="+localStorage.p_repID+"&repName="+localStorage.p_repName+"&townName="+localStorage.p_select_town+"&selectPOSM="+selectPOSM,
...
function leaveManageSUP(){
	...
	$('#menuBufferingImage').show();
	$("#agencySTRimage").show();
	//alert(apipath+"leaveManageSUPData?&repID="+localStorage.p_repID+"&select_town="+localStorage.p_select_town);
	$.ajax({
			type: 'POST',
			url:apipath+"leaveManageSUPData?&repID="+localStorage.p_repID+"&select_town="+localStorage.p_select_town,
...
function joinCallSubmitDataAll(){
	...
	//alert(apipath+"submitData_joinCallAllData?&select_town="+localStorage.p_select_town+"&repID="+localStorage.p_repID+"&repName="+localStorage.p_repName+"&mobileNo="+localStorage.p_mobileNo+"&joinNo1="+joinNo1+"&joinNo2="+joinNo2+"&joinNo3="+joinNo3+"&joinNo4="+joinNo4+"&shopeFornt_name="+shopeFornt_name+"&getFixedDisplayBefor_name="+getFixedDisplayBefor_name+"&getFixedDisplayAfter_name="+getFixedDisplayAfter_name+"&routeNameFixed="+rName+"&outletNameFixed="+outletCodeFxiedDisplayDala);
		$.ajax({
			type: 'POST',
			url:apipath+"submitData_joinCallAllData?&select_town="+localStorage.p_select_town+"&repID="+localStorage.p_repID+"&repName="+localStorage.p_repName+"&mobileNo="+localStorage.p_mobileNo+"&joinNo1="+joinNo1+"&joinNo2="+joinNo2+"&joinNo3="+joinNo3+"&joinNo4="+joinNo4+"&shopeFornt_name="+shopeFornt_name+"&getFixedDisplayBefor_name="+getFixedDisplayBefor_name+"&getFixedDisplayAfter_name="+getFixedDisplayAfter_name+"&routeNameFixed="+rName+"&outletNameFixed="+outletCodeFxiedDisplayDala,
